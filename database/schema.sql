...
GO
CREATE TABLE InterviewSessions (Id uniqueidentifier PRIMARY KEY, Topic nvarchar(100) NOT NULL, Track nvarchar(100) NOT NULL, Difficulty nvarchar(50) NOT NULL, CreatedAtUtc datetime2 NOT NULL, CompletedAtUtc datetime2 NULL, IsCompleted bit NOT NULL);
CREATE TABLE InterviewQuestions (Id int IDENTITY PRIMARY KEY, InterviewSessionId uniqueidentifier NOT NULL, QuestionNumber int NOT NULL, Domain nvarchar(100) NOT NULL, Concept nvarchar(100) NOT NULL, Difficulty nvarchar(50) NOT NULL, Text nvarchar(max) NOT NULL, CreatedAtUtc datetime2 NOT NULL, CONSTRAINT FK_Questions_Sessions FOREIGN KEY (InterviewSessionId) REFERENCES InterviewSessions(Id) ON DELETE CASCADE);
