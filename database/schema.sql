...
GO
CREATE TABLE InterviewSessions (Id uniqueidentifier PRIMARY KEY, Topic nvarchar(100) NOT NULL, Difficulty nvarchar(50) NOT NULL, CreatedAtUtc datetime2 NOT NULL, IsCompleted bit NOT NULL);
CREATE TABLE InterviewQuestions (Id int IDENTITY PRIMARY KEY, InterviewSessionId uniqueidentifier NOT NULL, QuestionNumber int NOT NULL, Text nvarchar(max) NOT NULL, CreatedAtUtc datetime2 NOT NULL, CONSTRAINT FK_Questions_Sessions FOREIGN KEY (InterviewSessionId) REFERENCES InterviewSessions(Id) ON DELETE CASCADE);
